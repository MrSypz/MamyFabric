...
public class FireballEntityRenderer extends EntityRenderer<FireballEntity> {
    ...
    public void render(FireballEntity entity, float yaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
        ...
        renderFireBall(matrices, vertexConsumers, entity, tickDelta);

        matrices.pop();
        super.render(entity, yaw, tickDelta, matrices, vertexConsumers, light);
    }

    private void renderFireBall(MatrixStack matrices, VertexConsumerProvider vertexConsumers, FireballEntity entity, float tickDelta) {
        matrices.push();

        VertexContext context = new VertexContext(matrices, vertexConsumers);
        Matrix4f matrix = matrices.peek().getPositionMatrix();

        float smoothTime = (entity.age + tickDelta) * 0.08f; // Different speed for fire
        float outerRotation = smoothTime * -1.5f * (float) Math.PI; // Different rotation speed
        float innerRotation = smoothTime * 2.2f * (float) Math.PI;

        VertexConsumer consumer = vertexConsumers.getBuffer(FLAME_ALPHA_LAYER);

        // Fire colors - orange to red gradient
        float alpha = 0.9f; // Slightly more opaque for fire
        float endAlpha = 0.0f;

        // Outer core - deep red
        float outerR = 1.0f, outerG = 0.2f, outerB = 0.0f;

        // Inner core - bright orange/yellow
        float innerR = 1.0f, innerG = 0.8f, innerB = 0.1f;

        // Draw outer layer (larger, red)
        drawRotatingBeamSides(context, consumer, matrix, 0.12f, HEIGHT,
                outerR, outerG, outerB, alpha * 0.7f, endAlpha, outerRotation);

        // Draw inner layer (smaller, orange-yellow)
        drawRotatingBeamSides(context, consumer, matrix, 0.07f, HEIGHT,
                innerR, innerG, innerB, alpha, endAlpha, innerRotation);

        // Add a very bright center core
        drawRotatingBeamSides(context, consumer, matrix, 0.03f, HEIGHT,
                1.0f, 1.0f, 0.8f, alpha * 1.2f, endAlpha, innerRotation * 1.5f);
