...
public class VertexContext {
    ...
    public void fillGradientWithTexture(VertexConsumer consumer, Matrix4f matrix,
                                        ...
                                        float x4, float y4, float z4, float u4, float v4, float r4, float g4, float b4, float a4) {
        consumer.vertex(matrix, x1, y1, z1).color(r1, g1, b1, a1).texture(u1, v1).light(LightmapTextureManager.MAX_LIGHT_COORDINATE);
        consumer.vertex(matrix, x2, y2, z2).color(r2, g2, b2, a2).texture(u2, v2).light(LightmapTextureManager.MAX_LIGHT_COORDINATE);
        consumer.vertex(matrix, x3, y3, z3).color(r3, g3, b3, a3).texture(u3, v3).light(LightmapTextureManager.MAX_LIGHT_COORDINATE);
        consumer.vertex(matrix, x4, y4, z4).color(r4, g4, b4, a4).texture(u4, v4).light(LightmapTextureManager.MAX_LIGHT_COORDINATE);
    }
}