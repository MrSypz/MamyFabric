...
public class ThunderBoltEntityRenderer extends EntityRenderer<ThunderBoltEntity> {
    ...
    private static final float HEIGHT = 1.2f; // Slightly taller than firebolt for lightning effect

    public ThunderBoltEntityRenderer(EntityRendererFactory.Context ctx) {
        super(ctx);
    }

    @Override
    public Identifier getTexture(ThunderBoltEntity entity) {
        return TEXTURE;
    }

    private static final RenderLayer THUNDER_ALPHA_LAYER = createThunderAlphaLayer();

    private static RenderLayer createThunderAlphaLayer() {
    ...
    @Override
    public void render(ThunderBoltEntity entity, float yaw, float tickDelta, MatrixStack matrices,
        ...
        renderThunderBolt(matrices, vertexConsumers, entity, tickDelta);

        matrices.pop();
        super.render(entity, yaw, tickDelta, matrices, vertexConsumers, light);
    }

    private void renderThunderBolt(MatrixStack matrices, VertexConsumerProvider vertexConsumers,
                                   ThunderBoltEntity entity, float tickDelta) {
        matrices.push();

        VertexContext context = new VertexContext(matrices, vertexConsumers);
        Matrix4f matrix = matrices.peek().getPositionMatrix();

        // Faster, more erratic rotation for lightning effect
        float smoothTime = (entity.age + tickDelta) * 0.15f;
        float rotation = smoothTime * 3.0f * (float) Math.PI; // Faster rotation for lightning

        VertexConsumer consumer = vertexConsumers.getBuffer(THUNDER_ALPHA_LAYER);

        // Inner core - bright electric blue
        float coreAlpha = 1.0f;
        float coreEndAlpha = 0.0f;
        float coreR = 0.4f, coreG = 0.8f, coreB = 1.0f; // Electric blue

        // Draw inner core
        drawRotatingBeamSides(context, consumer, matrix, 0.06f, HEIGHT,
                coreR, coreG, coreB, coreAlpha, coreEndAlpha, rotation);

        // Outer layer - lighter blue with more transparency
        float outerAlpha = 0.6f;
        float outerEndAlpha = 0.0f;
        float outerR = 0.7f, outerG = 0.9f, outerB = 1.0f; // Lighter electric blue

        // Draw outer layer with different rotation
        drawRotatingBeamSides(context, consumer, matrix, 0.08f, HEIGHT * 1.1f,
                outerR, outerG, outerB, outerAlpha, outerEndAlpha, -rotation * 0.7f);
