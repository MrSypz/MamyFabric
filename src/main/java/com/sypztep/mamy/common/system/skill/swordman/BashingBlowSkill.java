...
public class BashingBlowSkill extends Skill {
    ...
    public BashingBlowSkill(Identifier identifier) {
        super(identifier, "Bashing Blow", "Dash 5 blocks forward and deal massive damage to the first enemy hit",
                8f, 0.5f, ModClasses.SWORDMAN, 1, 1, 10, false, Mamy.id("skill/bashing_blow"));
    }

    @Override
    public float getResourceCost(int skillLevel) {
        // Cost: 8 resource levels 1-5, 16 resource levels 6-10
    ...
    protected SkillTooltipData getSkillTooltipData(PlayerEntity player, int skillLevel) {
        ...
        // Base damage from melee damage + 30% extra per level
        ...
        // 30% extra damage per level multiplier
    ...
    public boolean use(LivingEntity caster, int skillLevel) {
        ...
        Box damageArea = new Box(
                Math.min(startPos.x, endPos.x) - 1,
                Math.min(startPos.y, endPos.y),
                Math.min(startPos.z, endPos.z) - 1,
                Math.max(startPos.x, endPos.x) + 1,
                Math.max(startPos.y, endPos.y) + 2,
                Math.max(startPos.z, endPos.z) + 1
        );
        ...
        if (!targets.isEmpty()) {
            ...
            DamageSource damageSource = serverWorld.getDamageSources().create(
                    ModDamageTypes.BASHING_BLOW, player
            );
            ...
            serverWorld.spawnParticles(ParticleTypes.CRIT,
                    ...
                    15, 0.5, 0.5, 0.5, 0.1);
            serverWorld.spawnParticles(ModParticles.AIRHIKE,
