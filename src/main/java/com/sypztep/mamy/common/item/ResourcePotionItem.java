...
public class ResourcePotionItem extends Item {
    ...
    public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
        if (!world.isClient && user instanceof ServerPlayerEntity player) {
            ...
            if (resourceComponent != null) {
                ModEntityComponents.PLAYERCLASS.get(player).addResource(resourceComponent.resourceAmount());
                ...
                if (!player.isCreative() && resourceComponent.usingConvertsTo().isPresent()) {
                    ...
                    if (!player.getInventory().insertStack(convertedItem)) player.dropItem(convertedItem, false);
                }
            }
        }
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        ResourceComponent resourceComponent = stack.get(ModDataComponents.RESOURCE_RESTORE);
        ResourceType resourceType = ModEntityComponents.PLAYERCLASS.get(MinecraftClient.getInstance().player).getClassManager().getCurrentClass().getPrimaryResource();

        if (resourceComponent != null) {
            tooltip.add(Text.literal("Restores ").formatted(Formatting.GRAY)
                    .append(Text.literal((int) resourceComponent.resourceAmount() + " " + resourceType.getDisplayName())
