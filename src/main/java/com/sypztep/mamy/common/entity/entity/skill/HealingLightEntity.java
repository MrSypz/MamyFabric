...
public class HealingLightEntity extends PersistentProjectileEntity {
    ...
    private void renderCylinderOutline(Vec3d center) {
        ...
        for (int ring = 0; ring <= 1; ring++) {
            ...
            for (int i = 0; i < circumferencePoints; i++) {
                ...
                this.getWorld().addParticle(
                        new DustParticleEffect(color, 0.8f * intensity),
                        center.x + x,
                        center.y + y,
                        center.z + z,
                        0, 0.02, 0
        ...
        for (int i = 0; i < verticalLines; i++) {
            ...
            for (int h = 0; h < heightPoints; h++) {
                ...
                y += random.nextGaussian() * 0.08f;

                Vector3f color = new Vector3f(0.9f, 0.9f + random.nextFloat() * 0.1f, 0.5f);
                color.mul(intensity * 0.8f);

                this.getWorld().addParticle(
                        new DustParticleEffect(color, 0.6f * intensity),
                        center.x + x + random.nextGaussian() * 0.08,
                        center.y + y,
                        center.z + z + random.nextGaussian() * 0.08,
                        0, 0, 0
                );
            }
        }
    ...
    private void renderGroundCircle(Vec3d center) {
        ...
        for (int i = 0; i < circlePoints; i++) {
            ...
            x += random.nextGaussian() * 0.2f;
            z += random.nextGaussian() * 0.2f;

            Vector3f color = new Vector3f(1.0f, 0.9f, 0.4f);
            color.mul(intensity * 0.7f);

            this.getWorld().addParticle(
                    new DustParticleEffect(color, 1.2f * intensity),
                    groundPos.getX() + 0.5 + x,
                    groundPos.getY() + 0.1,
                    groundPos.getZ() + 0.5 + z,
                    0, 0, 0
            );
        }
    }
